...
def _parse_date_range(period):
    """根据时间筛选返回 (start_date_utc, end_date_utc)，基于北京时间计算"""
    bj_now = datetime.now(BJ_TZ)
    bj_today = bj_now.date()

    def _to_utc(d):
        """将北京日期转为 UTC naive datetime"""
        return datetime(d.year, d.month, d.day, tzinfo=BJ_TZ).astimezone(timezone.utc).replace(tzinfo=None)

    if period == 'today':
        return _to_utc(bj_today), _to_utc(bj_today + timedelta(days=1))
    elif period == 'yesterday':
        return _to_utc(bj_today - timedelta(days=1)), _to_utc(bj_today)
    elif period == 'this_week':
        monday = bj_today - timedelta(days=bj_today.weekday())
        month_1st = bj_today.replace(day=1)
        start = max(monday, month_1st)  # 本周不跨到上个月
        return _to_utc(start), _to_utc(bj_today + timedelta(days=1))
    elif period == 'last_week':
        monday = bj_today - timedelta(days=bj_today.weekday())
        start = monday - timedelta(days=7)
        return _to_utc(start), _to_utc(monday)
    elif period == 'this_month':
        start = bj_today.replace(day=1)
        return _to_utc(start), _to_utc(bj_today + timedelta(days=1))
    elif period == 'last_month':
        first_of_month = bj_today.replace(day=1)
        end = first_of_month
        start = (first_of_month - timedelta(days=1)).replace(day=1)
        return _to_utc(start), _to_utc(end)
    elif period == 'this_quarter':
        q = (bj_today.month - 1) // 3
        start = bj_today.replace(month=q * 3 + 1, day=1)
        return _to_utc(start), _to_utc(bj_today + timedelta(days=1))
    elif period == 'this_year':
        start = bj_today.replace(month=1, day=1)
        return _to_utc(start), _to_utc(bj_today + timedelta(days=1))
    elif period == 'last_7':
        return _to_utc(bj_today - timedelta(days=7)), _to_utc(bj_today + timedelta(days=1))
    elif period == 'last_30':
        return _to_utc(bj_today - timedelta(days=30)), _to_utc(bj_today + timedelta(days=1))
    else:
        # 默认本月
        start = bj_today.replace(day=1)
        return _to_utc(start), _to_utc(bj_today + timedelta(days=1))
